...
//////////////////////////////////////////////////////////
  ...
  if (rst)
    ...
    begin
        ...
        tx_data <= tx_data;
        tx_valid <= 1'b0; // Make sure to de-assert tx_valid by default
	    case(state)
	      RESET:begin
	          is_extend <= 1'b0;
              is_break <= 1'b1;
		  ...
		  SEND_CMD: begin
            if (busy == 1'b0 && tx_valid == 1'b0) begin
              tx_valid <= 1'b1; // Assert tx_valid to send command
            end
            // Remove the else if (busy == 1'b1) check to allow the FSM to proceed
            if (tx_valid == 1'b1 && rx_valid == 1'b1) begin
              state <= WAIT_ACK; // Move to the next state once the command is sent
            end
          end
          ...
          if (rx_valid == 1'b1) begin
            if (rx_data == RSP_ACK) begin
              // Handle different ACK scenarios based on the last command sent
              case (tx_data)
                CMD_RESET: state <= RESET_WAIT_BAT;
                CMD_SET_STATUS_LEDS: begin
                  tx_data <= {5'b00000, lock_status};
                  state <= SEND_CMD; // Prepare to send lock status next
                end
                default: state <= WAIT_KEYIN;
              endcase
              tx_valid <= 1'b0; // De-assert tx_valid after handling ACK
            end else begin
              state <= WAIT_KEYIN; // Handle unexpected data
            end
          end else if (err == 1'b1) begin
            state <= RESET; // Handle error conditions
          end
          // No else part needed; stay in this state until rx_valid or err changes
        end
